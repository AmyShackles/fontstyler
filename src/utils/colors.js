...
export function isOutlineRequired(color, background) {
    if (!/#[a-fA-F0-9]{4,6}/.test(color) && colorNameMap[color.toLowerCase()]) {
            color = colorNameMap[color.toLowerCase()];
    }
    if (!/#[a-fA-F0-9]{4,6}/.test(background) && colorNameMap[background.toLowerCase()]) {
            background = colorNameMap[background.toLowerCase()];
    }

    const textColor = hexToRGB(color);
    const backgroundColor = hexToRGB(background);
    const textLuminance = luminance(textColor.r, textColor.g, textColor.b);
